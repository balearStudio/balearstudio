...
// Renders each language with react-dom/server and writes real HTML into dist/:
// Spanish at the root, Catalan and English under /ca/ and /en/. Also writes
// sitemap.xml and robots.txt. No headless browser and no extra SSG dependency.
...
const { render, LANGUAGES, buildSitemap, robotsTxt } = await import(
...
for (const { code, path } of LANGUAGES) {
  const { appHtml, headHtml } = render(code)
...
console.log(`Prerendered ${LANGUAGES.length} pages: ${LANGUAGES.map((l) => l.path).join(' ')}`)
